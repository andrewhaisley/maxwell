...
{
    int32 oldStyleId = 0, oldColourId = 0;
    ...
    if ((geom->styleId == -1) || (geom->colourId == -1))
        return;
    ...
    bool setOutputLine)
{
    ...
    if (changeStyleStyles(styleId, currentDashStyleId,
        ...
        if (styleId != -1) {
            ...
            if (setOutputLine)
                outputLineStyle.dash_style = gDashStyle->style;
        } else {
            currentDashStyle = NULL;
            if (setOutputLine)
                outputLineStyle.dash_style.set(0, NULL, 0, -1);
        }
    ...
    bool& isSame)
{
    ...
    mx_line_style* lineStyle = nullptr;
    ...
    bool& isSame)
{
    ...
    mx_fill_style* fillStyle = nullptr;
    ...
    int colourId)
{
    ...
    if (isSame)
        outputLineStyle.line_type = mx_use_lstyle;
abort:
    return isSame;
}
    ...
    int colourId)
{
    ...
    // if the same style no need to reset the device
    if (isSame)
        outputFillStyle.type = mx_use_fstyle;
abort:
    return isSame;
}
    ...
    double& lineBack)
{
    ...
    if (capType == mx_cap_projecting)
        lineBack -= 0.5 * lineWidth;
