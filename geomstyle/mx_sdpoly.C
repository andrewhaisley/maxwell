    ...
    gmx_polypoint* gpolypoint)
{
    ...
    if (drawSelectedPolypoint(err, gpolypoint))
        return;
    MX_ERROR_CHECK(err);
    ...
    int lineColourId)
{
    ...
    if ((lineStyleId == -1) || (lineColourId == -1))
        return;
    ...
    if ((!idocpp.pp.get_closed()) && (numPoints > 1) && (thisLineStyle->getArrows(startArrow, endArrow))) {
        if (startArrow) {
    ...
    gmx_polypoint* gpolypoint)
{
    ...
    if (gpolypoint->selectCount == MX_GEOM_SNAP_NONE)
        return FALSE;
